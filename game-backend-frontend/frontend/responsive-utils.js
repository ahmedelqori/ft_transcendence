export class GameRenderer {
  ...
  xToPercent(xPixels) {
    return (xPixels / this.canvasWidth) * 100;
  }
  ...
  drawGameOver(context, gameState) {
    ...
    const winner = gameState.winner === "left" ? "Left Player" : "Right Player";
  ...
  drawBallTrajectory(context, gameState) {
    if (!gameState.inProgress || !gameState.ball.xDir || !gameState.ball.yDir)
      return;
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (
        x <= this.gameConfig.leftPaddleX + this.gameConfig.paddleWidth + 1 ||
  ...
  renderGame(gameState) {
    ...
    this.drawPaddle(
      context,
      this.gameConfig.leftPaddleX,
      gameState.paddles.left
    );
    this.drawPaddle(
      context,
      this.gameConfig.rightPaddleX - this.gameConfig.paddleWidth,
      gameState.paddles.right
    );
    this.drawBall(context, gameState.ball.x, gameState.ball.y);

    // Draw the score on the canvas itself
    this.drawScore(context, gameState);
  ...
  // Fix the score drawing method - remove backward compatibility
  drawScore(context, gameState) {
    ...
    // Only use left/right naming convention
    const leftScore = gameState.score.left || 0;
    const rightScore = gameState.score.right || 0;

    // Draw score as a single string for better alignment
    context.fillText(
   ...
   * This function has issues - simplify by removing it
   * We'll use the direct calculation in handleMouseMove instead
   */
  convertScreenToGameCoordinates(screenX, screenY) {
    const rect = this.canvas.getBoundingClientRect();

    // Account for position within canvas
