...
class VaultService {
  ...
  async unwrapToken(wrapToken) {
    try {
      ...
      throw new Error(
        `Unwrap failed: ${error.response?.data?.errors || error.message}`
      );
    }
  }
  ...
  async login(roleId, secretId) {
    try {
      ...
      throw new Error(
        `Login failed: ${error.response?.data?.errors || error.message}`
      );
    }
  }
...
export async function loadSecrets() {
  ...
  const secretPaths = [
    { key: "ORIGIN_S2S", path: "secret/data/oauth/S2S", field: "ORIGIN" },
    {
      ...
      field: "SOCIAL_AUTH_GOOGLE_OAUTH2_KEY",
    },
    {
      key: "ORIGIN_42",
      path: "secret/data/oauth/42",
      field: "SOCIAL_AUTH_42_OAUTH2_KEY",
    },
    {
      key: "SECRET_GOOGLE",
      path: "secret/data/oauth/google",
    ...
    },
    {
      key: "SECRET_42",
      path: "secret/data/oauth/42",
      field: "SOCIAL_AUTH_42_OAUTH2_SECRET",
    },
    {
      key: "JWT_PRIVATE",
