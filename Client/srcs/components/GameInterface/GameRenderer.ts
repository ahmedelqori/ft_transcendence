...
export class ResponsiveCanva {
  ...
  setup(): void {
    ...
    if (this.canvas.width !== width || this.canvas.height !== height) {
      console.log(`[ResponsiveCanva] Setting canvas dimensions: ${width.toFixed(0)}x${height.toFixed(0)}`);
...
interface GameRendererMethods {
  ...
  renderGame(): void;
  drawCenterLine(ctx: CanvasRenderingContext2D, responsiveCanva: ResponsiveCanva): void;
  drawBall(ctx: CanvasRenderingContext2D, x: number, y: number, responsiveCanva: ResponsiveCanva): void;
  drawPaddle(ctx: CanvasRenderingContext2D, x: number, y: number, responsiveCanva: ResponsiveCanva, color?: string): void;
  drawScore(ctx: CanvasRenderingContext2D, gameState: GameState, responsiveCanva: ResponsiveCanva): void;
  drawBallTrajectory(ctx: CanvasRenderingContext2D, gameState: GameState, responsiveCanva: ResponsiveCanva): void;
...
export const GameRenderer = defineComponent<GameRendererState, GameRendererProps>({
  ...
  initCanvas(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods): boolean {
    try {
      if (!this.props.gameConfig) {
        console.log("[GameRenderer] Waiting for game configuration");
        return false;
      ...
      const responsiveCanva = new ResponsiveCanva(canvas, this.props.gameConfig);
  ...
  startAnimationLoop(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods) {
    if (!this.props.gameConfig) {
      console.log("[GameRenderer] Not starting animation loop - waiting for game config");
      return;
    }
    const animate = () => {
      this.renderGame();
  ...
  renderGame(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods) {
    ...
    try {
      ...
      if (gameState.ball) {
        this.drawBall(ctx, gameState.ball.x, gameState.ball.y, responsiveCanva!);
      }
  ...
  setupDrawing(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, responsiveCanva: ResponsiveCanva): void {
  ...
  drawCenterLine(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, responsiveCanva: ResponsiveCanva) {
  ...
  drawBall(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, x: number, y: number, responsiveCanva: ResponsiveCanva) {
  ...
  drawPaddle(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, x: number, y: number, responsiveCanva: ResponsiveCanva, color: string = "#FFFFFF") {
    ctx.fillStyle = color;
    ...
    const paddleWidth = responsiveCanva.xToPixels(this.props.gameConfig.paddleWidth);
    const paddleHeight = responsiveCanva.yToPixels(this.props.gameConfig.paddleHeight);
  ...
  drawScore(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, gameState: GameState, responsiveCanva: ResponsiveCanva) {
  ...
  drawBallTrajectory(this: IComponent<GameRendererState, GameRendererProps> & GameRendererMethods, ctx: CanvasRenderingContext2D, gameState: GameState, responsiveCanva: ResponsiveCanva) {
