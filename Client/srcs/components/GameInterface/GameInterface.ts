...
const GameInterface = defineComponent<GameInterfaceState>({
  ...
  onUnmounted(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    ...
    if (this.state.beforeUnloadHandler) {
      window.removeEventListener("beforeunload", this.state.beforeUnloadHandler);
    }
  ...
  setupSocketListeners(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    ...
    handlers.onGameState = (data: GameStateData) => {
      ...
      if (data.state === "readyToStart") {
        console.log("[GameInterface] Ready to start received from server, starting countdown for all players");
    ...
    handlers.onPlayerJoined = (data: PlayerJoinedData) => {
      if (data.position) {
        ...
        console.log(`[GameInterface] Joined as ${data.position} player. ${playersCount}/2 players connected.`);
      } else {
        const playersCount = Object.keys(data.players).length;
        ...
        console.log(`[GameInterface] Player joined. ${playersCount}/2 players connected.`);
      }
    };
    ...
    handlers.onReconnect = (data: ReconnectData) => {
      if (data.gameState) {
        ...
        console.log(`[GameInterface] Reconnected to game as ${playerPosition} player`);
      } else {
        console.log(`[GameInterface] Player reconnected (position: ${data.position})`);
        
        if (this.state.opponentDisconnected && data.position !== socketManager.getPlayerPosition()) {
          this.updateState({ 
            opponentDisconnected: false,
            showDisconnectedOverlay: false,
            showPausedOverlay: this.state.gameState?.state === GameStates.PAUSED
          });
        }
      }
    };
    ...
    Object.entries(handlers).forEach(([event, handler]) => {
      socketManager.addEventListener(event as keyof SocketManager['listeners'], handler as any);
    });
  ...
  removeSocketListeners(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    const { socketManager, socketEventHandlers } = this.state;
    if (!socketManager) return;
    
    Object.entries(socketEventHandlers).forEach(([event, handler]) => {
      socketManager.removeEventListener(event as keyof SocketManager['listeners'], handler as any);
    });
  ...
  disconnectFromGame(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    const { socketManager } = this.state;
    if (socketManager && socketManager.getIsConnected()) {
  ...
  togglePauseResume(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    const { socketManager, gameState } = this.state;
    
    if (!socketManager || !socketManager.getIsConnected() || !gameState) {
      console.log("[GameInterface] Cannot pause/resume: Not connected or no game in progress");
      return;
    }
  ...
  startCountdown(this: IComponent<GameInterfaceState> & GameInterfaceMethods, seconds: number) {
    if (this.state.countdownTimerId !== null) {
      clearInterval(this.state.countdownTimerId);
    }
  ...
  render(this: IComponent<GameInterfaceState> & GameInterfaceMethods) {
    ...
    const score = gameState?.score ? {
      player: gameState.score[this.state.playerPosition === "left" ? "left" : "right"] || 0,
      opponent: gameState.score[this.state.playerPosition === "left" ? "right" : "left"] || 0
    } : { player: 0, opponent: 0 };
    ...
    const gameCanvasWithOverlays = createElement(
      ...
      [
        ...
        createElement(DisconnectedOverlay, { 
          visible: showDisconnectedOverlay && !showWaitingConfigOverlay && !showWaitingOpponentOverlay && !showCountdownOverlay
        }),
        
        createElement(VictoryOverlay, { 
          visible: showVictoryOverlay && !showDisconnectedOverlay && !showWaitingConfigOverlay && !showWaitingOpponentOverlay, 
          score: score,
          onPlayAgain: this.handlePlayAgain.bind(this)
        }),
        
        createElement(GameOverLossOverlay, { 
          visible: showDefeatOverlay && !showDisconnectedOverlay && !showWaitingConfigOverlay && !showWaitingOpponentOverlay, 
          score: score 
        }),
        
        createElement(GamePausedOverlay, { 
          visible: showPausedOverlay && !showDisconnectedOverlay && !showVictoryOverlay && !showDefeatOverlay 
                  && !showWaitingConfigOverlay && !showWaitingOpponentOverlay && !showCountdownOverlay,
